...
import {CustomerTypeSearch} from './customer-type.search.model';
...
currentAccount: any;
    ...
    constructor(
        ...
        this.customerTypeSearch = new CustomerTypeSearch();

        this.customerTypeSearch.code = this.activatedRoute.snapshot && this.activatedRoute.snapshot.params['code'] ?
                        this.activatedRoute.snapshot.params[' code'] : '';
        this.customerTypeSearch.name = this.activatedRoute.snapshot && this.activatedRoute.snapshot.params['name'] ?
                        this.activatedRoute.snapshot.params[' name'] : '';
        this.customerTypeSearch.description = this.activatedRoute.snapshot && this.activatedRoute.snapshot.params['description'] ?
                        this.activatedRoute.snapshot.params[' description'] : '';
        this.customerTypeSearch.companyId = this.activatedRoute.snapshot && this.activatedRoute.snapshot.params['companyId'] ?
    ...
    loadAll() {
         ...
         sort: this.sort(),
         code : this.customerTypeSearch.code,
         name : this.customerTypeSearch.name,
         description : this.customerTypeSearch.description,
           companyId : this.customerTypeSearch.companyId,
         };

        this.customerTypeService.searchExample(obj).subscribe(
    ...
    transition() {
        this.router.navigate(['/customer-management/customer-type'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/customer-management/customer-type', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    public deleteItem(id:number){
        this.customerTypePopupService
            .open(CustomerTypeDeleteDialogComponent as Component, id);
    }
}
