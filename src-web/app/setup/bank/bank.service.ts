...
export class BankService {
    ...
    private resourceSearchExampleUrl = SERVER_API_URL + 'api/_search_example/banks';
    ...
    search(req?: any): Observable<HttpResponse<Bank[]>> {
        const options = createRequestOption(req);
        return this.http.get<Bank[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Bank[]>) => this.convertArrayResponse(res));

    }
    searchExample(req? : any): Observable<HttpResponse<Bank[]>> {
            const options = createRequestOption(req);
        return this.http.get<Bank[]>(this.resourceSearchExampleUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Bank[]>) => this.convertArrayResponse(res));
    }
