...
public class QuotationServiceImpl implements QuotationService {
	...
	public QuotationServiceImpl(QuotationRepository quotationRepository, QuotationMapper quotationMapper,
			QuotationSearchRepository quotationSearchRepository, CompanySearchRepository companySearchRepository,
	...
	public Page<QuotationDTO> searchExample(QuotationSearchDTO searchDto, Pageable pageable) {
		...
		if (searchDto.getCompanyId() != null) {
			boolQueryBuilder.must(QueryBuilders.matchQuery("isActive", searchDto.isIsActive()));
		}
		...
		List<QuotationDTO> quotationList = StreamSupport.stream(quotationPage.spliterator(), false)
				.map(quotationMapper::toDto).collect(Collectors.toList());
