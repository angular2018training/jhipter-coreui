...
import vn.nextlogix.service.RegionQueryService;
...
public class RegionResource {
    ...
    @GetMapping("/_search_example/regions")
    @Timed
    public ResponseEntity<List<RegionDTO>> searchExampleRegions(RegionSearchDTO searchDTO , Pageable pageable) throws ApplicationException {
        log.debug("REST request to search example for a page of Regions for searchDTO {}", searchDTO);
        Page<RegionDTO> page = regionService.searchExample(searchDTO, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(searchDTO, page, "/api/_search_example/regions");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
