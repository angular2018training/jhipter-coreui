...
public class CustomerServicesResource {
	...
	public CustomerServicesResource(CustomerServicesService customerServicesService,
			CustomerServicesQueryService customerServicesQueryService) {
	 ...
	 *         customerServicesDTO, or with status 400 (Bad Request) if the
	 *         customerServices has already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PostMapping("/customer-services")
	@Timed
	public ResponseEntity<CustomerServicesDTO> createCustomerServices(
			@Valid @RequestBody CustomerServicesDTO customerServicesDTO) throws URISyntaxException {
		log.debug("REST request to save CustomerServices : {}", customerServicesDTO);
		if (customerServicesDTO.getId() != null) {
			throw new BadRequestAlertException("A new customerServices cannot already have an ID", ENTITY_NAME,
					"idexists");
		}
	 ...
	 *         customerServicesDTO, or with status 400 (Bad Request) if the
	 *         customerServicesDTO is not valid, or with status 500 (Internal Server
	 *         Error) if the customerServicesDTO couldn't be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/customer-services")
	@Timed
	public ResponseEntity<CustomerServicesDTO> updateCustomerServices(
			@Valid @RequestBody CustomerServicesDTO customerServicesDTO) throws URISyntaxException {
	 ...
	 *         customerServices in body
	 */
	@GetMapping("/customer-services")
	@Timed
	public ResponseEntity<List<CustomerServicesDTO>> getAllCustomerServices(CustomerServicesCriteria criteria,
			Pageable pageable) {
	 ...
	 *         customerServicesDTO, or with status 404 (Not Found)
	 */
	...
	/**
	 * SEARCH /_search/customer-services?query=:query : search for the
	 * customerServices corresponding to the query.
	 *
	...
	@Timed
	public ResponseEntity<List<CustomerServicesDTO>> searchCustomerServices(@RequestParam String query,
			Pageable pageable) {
		...
		HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page,
				"/api/_search/customer-services");
		return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
	}

	@GetMapping("/_search_example/customer-services")
	@Timed
	public ResponseEntity<List<CustomerServicesDTO>> searchExampleCustomerServices(CustomerServicesSearchDTO searchDTO,
		...
		HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(searchDTO, page,
				"/api/_search_example/customer-services");
		return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
	}
