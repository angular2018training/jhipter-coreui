...
import vn.nextlogix.service.dto.CustomerWarehouseWrapperDTO;
...
public class CustomerWarehouseResource {
	...
	public CustomerWarehouseResource(CustomerWarehouseService customerWarehouseService,
			CustomerWarehouseQueryService customerWarehouseQueryService, WarehouseService warehouseService) {
	 ...
	 *         customerWarehouseDTO, or with status 400 (Bad Request) if the
	 *         customerWarehouse has already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PostMapping("/customer-warehouses")
	@Timed
	public ResponseEntity<CustomerWarehouseDTO> createCustomerWarehouse(
			@Valid @RequestBody CustomerWarehouseWrapperDTO customerWarehouseWrapperDTO) throws URISyntaxException {
		CustomerWarehouseDTO customerWarehouseDTO = customerWarehouseWrapperDTO.getCustomerWarehouseDTO();
		WarehouseDTO warehouseDTO = customerWarehouseWrapperDTO.getWarehouseDTO();

		log.debug("REST request to save CustomerWarehouse : {}", customerWarehouseDTO);
		if (customerWarehouseDTO.getId() != null) {
			throw new BadRequestAlertException("A new customerWarehouse cannot already have an ID", ENTITY_NAME,
					"idexists");
		}
		WarehouseDTO savedWarehouse = warehouseService.save(warehouseDTO);
		customerWarehouseDTO.setWarehouseId(savedWarehouse.getId());
	 ...
	 *         customerWarehouseDTO, or with status 400 (Bad Request) if the
	 *         customerWarehouseDTO is not valid, or with status 500 (Internal
	 *         Server Error) if the customerWarehouseDTO couldn't be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/customer-warehouses")
	@Timed
	public ResponseEntity<CustomerWarehouseDTO> updateCustomerWarehouse(
			@Valid @RequestBody CustomerWarehouseWrapperDTO customerWarehouseWrapperDTO) throws URISyntaxException {
		CustomerWarehouseDTO customerWarehouseDTO = customerWarehouseWrapperDTO.getCustomerWarehouseDTO();
		log.debug("REST request to update CustomerWarehouse : {}", customerWarehouseDTO);
		if (customerWarehouseDTO.getId() == null) {
			return createCustomerWarehouse(customerWarehouseWrapperDTO);
		}
		WarehouseDTO warehouseDTO = customerWarehouseWrapperDTO.getWarehouseDTO();
	 ...
	 *         customerWarehouses in body
	 */
	@GetMapping("/customer-warehouses")
	@Timed
	public ResponseEntity<List<CustomerWarehouseDTO>> getAllCustomerWarehouses(CustomerWarehouseCriteria criteria,
			Pageable pageable) {
	 ...
	 *         customerWarehouseDTO, or with status 404 (Not Found)
	 */
	...
	/**
	 * SEARCH /_search/customer-warehouses?query=:query : search for the
	 * customerWarehouse corresponding to the query.
	 *
	...
	@Timed
	public ResponseEntity<List<CustomerWarehouseDTO>> searchCustomerWarehouses(@RequestParam String query,
			Pageable pageable) {
		...
		HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page,
				"/api/_search/customer-warehouses");
		return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
	}

	@GetMapping("/_search_example/customer-warehouses")
	@Timed
	public ResponseEntity<List<CustomerWarehouseDTO>> searchExampleCustomerWarehouses(
			CustomerWarehouseSearchDTO searchDTO, Pageable pageable) throws ApplicationException {
		...
		HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(searchDTO, page,
				"/api/_search_example/customer-warehouses");
		return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
	}
