...
public class CustomerLegalResource {
	...
	public CustomerLegalResource(CustomerLegalService customerLegalService,
			CustomerLegalQueryService customerLegalQueryService, CustomerService customerService) {
	 ...
	 *         customerLegalDTO, or with status 400 (Bad Request) if the
	 *         customerLegal has already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PostMapping("/customer-legals")
	@Timed
	public ResponseEntity<CustomerLegalDTO> createCustomerLegal(@Valid @RequestBody CustomerLegalDTO customerLegalDTO,
		...
		if (customerLegalDTO.getId() != null) {
			throw new BadRequestAlertException("A new customerLegal cannot already have an ID", ENTITY_NAME,
					"idexists");
		}
		...
		customerDTO.setLegalId(result.getId());
	 ...
	 *         customerLegalDTO, or with status 400 (Bad Request) if the
	 *         customerLegalDTO is not valid, or with status 500 (Internal Server
	 *         Error) if the customerLegalDTO couldn't be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/customer-legals")
	@Timed
	public ResponseEntity<CustomerLegalDTO> updateCustomerLegal(@Valid @RequestBody CustomerLegalDTO customerLegalDTO,
			@RequestParam(value = "customerId") Long customerId) throws URISyntaxException {
		log.debug("REST request to update CustomerLegal : {}", customerLegalDTO);
		if (customerLegalDTO.getId() == null) {
			return createCustomerLegal(customerLegalDTO, customerId);
		}
		CustomerLegalDTO result = customerLegalService.save(customerLegalDTO);
	 ...
	 *         customerLegals in body
	 */
	@GetMapping("/customer-legals")
	@Timed
	public ResponseEntity<List<CustomerLegalDTO>> getAllCustomerLegals(CustomerLegalCriteria criteria,
			Pageable pageable) {
	 ...
	 *         customerLegalDTO, or with status 404 (Not Found)
	 */
	...
	/**
	 * SEARCH /_search/customer-legals?query=:query : search for the customerLegal
	...
	public ResponseEntity<List<CustomerLegalDTO>> searchCustomerLegals(@RequestParam String query, Pageable pageable) {
		...
		HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page,
				"/api/_search/customer-legals");
		return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
	}

	@GetMapping("/_search_example/customer-legals")
	@Timed
	public ResponseEntity<List<CustomerLegalDTO>> searchExampleCustomerLegals(CustomerLegalSearchDTO searchDTO,
		...
		HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(searchDTO, page,
				"/api/_search_example/customer-legals");
		return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
	}
